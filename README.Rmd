...
# Run report scripts in parallel using callr
reportScripts <- sort(list.files("R", pattern = "^report-.*\\.R$", full.names = TRUE))
bgProcesses <- lapply(reportScripts, function(script) {
  callr::r_bg(function(scriptPath) source(scriptPath), args = list(scriptPath = script))
})
# Wait for all report scripts to complete
lapply(bgProcesses, function(p) p$wait())

# Run tests without re-sourcing R/ scripts (already executed above)
...
testResultsRaw <- testResultsRaw[order(testResultsRaw$context), ]
